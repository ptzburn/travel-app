...
const CalendarHeading = <T extends ValidComponent = "h2">(
  ...
  const [local, others] = splitProps(props as CalendarHeadingProps, [
    "class",
  ]);
  return (
...
const CalendarGridHeadRow: Component<ComponentProps<"tr">> = (props) => {
  const [local, others] = splitProps(props, ["class"]);
  return <tr class={cn("flex justify-center", local.class)} {...others} />;
};
...
const CalendarGridBodyRow: Component<ComponentProps<"tr">> = (props) => {
  ...
  return (
    <tr
      class={cn("mt-2 flex w-full justify-center", local.class)}
      {...others}
    />
  );
};
